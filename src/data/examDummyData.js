export const FACULTY_CATALOG = [
  ...
  { id: "fac_bsc_csit", code: "BSC CSIT", structureType: "semester", maxLevel: 8 },
];
...
export function getLevelOptionsForFacultyCode(code) {
  ...
  if (!fac) return [];
  const max = Math.max(1, Math.min(fac.maxLevel, fac.structureType === "semester" ? 8 : 5));
  return Array.from({ length: max }, (_, i) => {
    const level = i + 1;
    return { value: String(level), label: getLevelLabel(fac.structureType, level) };
  });
}
...
export function buildDummyStudents() {
  ...
  for (const fac of FACULTY_CATALOG) {
    const levelCount = Math.min(fac.maxLevel, fac.structureType === "semester" ? 8 : 4);
    ...
    exams: [
      {
        ...
        items: [
          { id: "i-1", date: "2026-04-10", time: "09:00", subject: "Data Structures" },
          { id: "i-2", date: "2026-04-12", time: "10:00", subject: "Database Systems" },
        ],
      },
    ],
  },
    ...
    exams: [
      {
        ...
        items: [
          { id: "i-1", date: "2026-03-20", time: "08:30", subject: "Business Law" },
          { id: "i-2", date: "2026-03-22", time: "09:30", subject: "Marketing" },
        ],
      },
    ],
  },
